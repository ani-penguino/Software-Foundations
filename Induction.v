...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    Proof:
      ...
      Base Case: 
          a = 0 First, suppose a = 0. 
          We must show that 0 + b = b + 0. 
          By the definition of addition, 0 + b simplifies directly to b. 
          Similarly, b + 0 also simplifies to b by the definition of addition. 
          Thus, 0 + b = b = b + 0.

      Inductive Step: 
          a = S(a') Next, suppose a = S(a'), where a' is a natural number, 
          and by the induction hypothesis, assume a' + b = b + a'. 
          We must now show that (S(a')) + b = b + (S(a')). 
          By the definition of addition, (S(a')) + b simplifies to S(a' + b). 
          Using the induction hypothesis, we rewrite a' + b as b + a', giving us S(a' + b) = S(b + a'). 
          By the definition of addition, S(b + a') is just b + S(a'). 
          Thus, S(a' + b) = b + S(a'), and so (S(a')) + b = b + (S(a')).

      Conclusion: 
          By the principle of mathematical induction, 
          we conclude that addition is commutative for all natural numbers a and b. 
          That is, for any a, b in the set of natural numbers, a + b = b + a. Qed.
*)
...
Theorem mul_comm_helper : forall n m, m * S n = m + m * n.
Proof.
  intros n m.
  induction m as [|p].
    simpl. 
...
Proof.
  intros. induction p as [|q].
    ...
    rewrite -> mul_comm_helper. rewrite -> mul_comm_helper. rewrite mul_comm_helper.
    rewrite -> add_shuffle3. rewrite -> IHq.
    rewrite <- add_assoc. rewrite <- add_assoc. rewrite -> add_shuffle3.
    reflexivity.
Qed.
